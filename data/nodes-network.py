...
def plot(ax, df, network, app):
    ...
    plot_df = (
        df.group_by(["setting", "num_nodes"])  # changed: use Polars group_by
        .agg(
            [
                pl.col("traffic").mean().alias("mean"),
                pl.col("traffic").std().alias("std"),
            ]
        )
    ...
    settings = sorted(
        plot_df["setting"].unique().to_list()
    )  # changed: get unique settings
